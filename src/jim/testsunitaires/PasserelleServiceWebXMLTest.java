...
public class PasserelleServiceWebXMLTest {
	...
	public void testCreerUnUtilisateur() {
		...
		assertEquals("Erreur : adresse mail incorrecte ou dÃ©jÃ  existante.", msg);
	...
	public void testSupprimerUnUtilisateur() {
		...
		assertEquals("Erreur : suppression impossible ; cet utilisateur possÃ¨de encore des traces.", msg);	
	...
	public void testDemanderUneAutorisation() {
		...
		assertEquals("Erreur : donnï¿½es incomplï¿½tes.", msg);
	...
	public void testRetirerUneAutorisation() {
		...
		assertEquals("Erreur : l'autorisation n'ï¿½tait pas accordï¿½e.", msg);	
		
		msg = PasserelleServicesWebXML.retirerUneAutorisation("neon", Outils.sha1("mdputilisateur"), "oxygen", "coucou");
		assertEquals("Autorisation supprimï¿½e ; oxygen va recevoir un courriel de notification.", msg);	
		
		msg = PasserelleServicesWebXML.retirerUneAutorisation("neon", Outils.sha1("mdputilisateur"), "photon", "");
		assertEquals("Autorisation supprimï¿½e.", msg);
	...
	public void testEnvoyerPosition() throws ParseException {
		...
		assertEquals("Erreur : le numï¿½ro de trace n'existe pas.", msg);
		
		lePoint = new PointDeTrace(22, 0, 48.15, -1.68, 50, laDate, 80);
		msg = PasserelleServicesWebXML.envoyerPosition("europa", Outils.sha1("mdputilisateur"), lePoint);
		assertEquals("Erreur : le numï¿½ro de trace ne correspond pas ï¿½ cet utilisateur.", msg);	
		
		lePoint = new PointDeTrace(4, 0, 48.15, -1.68, 50, laDate, 80);
		msg = PasserelleServicesWebXML.envoyerPosition("europa", Outils.sha1("mdputilisateur"), lePoint);
		assertEquals("Point crï¿½ï¿½.", msg);	
